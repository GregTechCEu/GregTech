...
void main(){
    vec3 rectangle = getSquare(textureCoords, vec2(-0.3, 0.21)) * vec3(2.0, 12.0, 30.0) * 2.0;
    vec3 circle = getCircle(textureCoords, vec2(0.2, 0.29)) * vec3(30.0, 12.0, 2.0) * 2.0;
    vec3 triangle = getTriangle(textureCoords, vec2(0.0, -0.23)) * vec3(2.0, 30.0, 2.0) * 2.0;
    vec3 color = rectangle + circle + triangle;
    color = pow(getTexture(textureCoords), vec3(2.2)) * 10.0;

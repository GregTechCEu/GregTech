            ...
            if (this.screen.getWorld().isRemote) { // check connections
                ...
                if (holder != null && holder.getMetaTileEntity() instanceof MultiblockControllerBase) {
                    ...
                    if (entity.isStructureFormed()) {
                        if (!isValid) {
                            PatternMatchContext result = entity.structurePattern.checkPatternFastAt(entity.getWorld(), entity.getPos(), entity.getFrontFacing().getOpposite(), entity.getUpwardsFacing());
                            if (result != null) {

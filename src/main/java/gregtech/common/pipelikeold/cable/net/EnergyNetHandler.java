...
public class EnergyNetHandler implements IEnergyContainer, IPipeNetHandler {
    ...
    protected static final PipeNetNode<Insulation, WireProperties, NetFlowEdge> FAKE_SOURCE = new PipeNetNode<>(
    ...
    private final Map<PipeNetNode<Insulation, WireProperties, NetFlowEdge>, NodeLossResult> lossResultCache = new Object2ObjectOpenHashMap<>();
    ...
    public long getInputPerSec() {
        AbstractGroupData data = net.getGroup(cable.getPipePos()).getData();
    ...
    private long distributionRespectCapacity(EnumFacing side, long voltage, long amperage, long queryTick,
                                             Iterator<PipeNetPath<Insulation, WireProperties, NetFlowEdge>> paths,
                                             SimulatorKey simulator) {
        long availableAmperage = amperage;
        mainloop:
        while (paths.hasNext()) {
            PipeNetPath<Insulation, WireProperties, NetFlowEdge> path = paths.next();
            ...
            while (iterator.hasNext()) {
                PipeNetPath.FacedPipeNetPath<Insulation, WireProperties, NetFlowEdge> facedPath = path
                ...
                List<PipeNetNode<Insulation, WireProperties, NetFlowEdge>> nodeList = facedPath.getNodeList();
                ...
                for (int j = 0; j < nodeList.size(); j++) {
                    ...
                    PipeNetNode<Insulation, WireProperties, NetFlowEdge> target = nodeList.get(j);
                    // amperage capping
    ...
    private boolean calculateVoltageLoss(long voltage, SimulatorKey simulator,
                                         ...
                                         PipeNetNode<Insulation, WireProperties, NetFlowEdge> target) {
    ...
    protected NodeLossResult getOrGenerateLossResult(PipeNetNode<Insulation, WireProperties, NetFlowEdge> node,

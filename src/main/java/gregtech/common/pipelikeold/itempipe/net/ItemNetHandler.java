...
public class ItemNetHandler implements IItemHandler, IPipeNetHandler {
    ...
    public ItemStack insertFirst(ItemStack stack, boolean simulate) {
        for (Iterator<PipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge>> it = net.getPaths(pipe); it.hasNext();) {
            PipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> inv = it.next();
    ...
    public ItemStack insertRoundRobin(ItemStack stack, boolean simulate, boolean global) {
        Iterator<PipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge>> routePaths = net.getPaths(pipe);
        if (!routePaths.hasNext()) return stack;
        List<PipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge>> routePathsCopy = new ObjectArrayList<>();
     ...
     * @return remainder
     */
    private ItemStack insertToHandlers(List<PipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge>> copy, ItemStack stack,
                                       boolean simulate) {
        Iterator<PipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge>> routePathIterator = copy.listIterator();
        int inserted = 0;
        ...
        while (routePathIterator.hasNext()) {
            PipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> routePath = routePathIterator.next();
            Iterator<EnumFacing> iterator = routePath.getFacingIterator();
            while (iterator.hasNext()) {
                PipeNetPath.FacedPipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> facedPipeNetPath = routePath
                ...
                int r = insert(facedPipeNetPath, toInsert, simulate).getCount();
                if (r < amount) {
                    inserted += (amount - r);
                }
                if (r == 1 && c == 0 && amount == 1) {
    ...
    private ItemStack insertToHandlersEnhanced(List<PipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge>> copy,
        ...
        for (PipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> inv : copy) {
            Iterator<EnumFacing> iterator = inv.getFacingIterator();
            while (iterator.hasNext()) {
                PipeNetPath.FacedPipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> faceInv = inv
                        .withFacing(iterator.next());
                simStack = stack.copy();
    ...
    public ItemStack insert(PipeNetPath.FacedPipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> routePath, ItemStack stack,
                            boolean simulate) {
    ...
    private void transferTo(PipeNetPath.FacedPipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> routePath, boolean simulate,
                            int amount) {
        if (simulate)
    ...
    private boolean contains(PipeNetPath.FacedPipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> routePath,
                             boolean simulate) {
    ...
    private int didTransferTo(PipeNetPath.FacedPipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> routePath,
                              boolean simulate) {
        if (simulate)
    ...
    private static class EnhancedRoundRobinData {
        ...
        private final PipeNetPath.FacedPipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> routePath;
        private final int maxInsertable;
        private int transferred;
        private int toTransfer = 0;

        private EnhancedRoundRobinData(PipeNetPath.FacedPipeNetPath<ItemPipeType, ItemPipeProperties, NetEdge> routePath,

...
public class RecipeChartApp extends AbstractApplication implements IRecipeTransferHandlerWidget {
    ...
    public AbstractApplication initApp() {
        if (isClient) {
            this.tabGroup = new TabGroup<>(0, 10, new CustomTabListRenderer(TerminalTheme.COLOR_F_2,
                    TerminalTheme.COLOR_B_3, 333 / getMaxPages(), 10));
    ...
    private RGContainer addTab(String name) {
        ...
        tabGroup.addTab(new IGuiTextureTabInfo(new TextTexture(name, -1).setWidth(333 / getMaxPages() - 5)
                .setType(tabGroup.getAllTag().isEmpty() ? TextTexture.TextType.ROLL : TextTexture.TextType.HIDE), name),
                container);
        return container;
    }
    ...
    public List<IMenuComponent> getMenuComponents() {
                ...
                .setHoverText("terminal.component.new_page").setClickConsumer(cd -> {
                    ...
                    if (tabGroup.getAllTag().size() < getMaxPages()) {
                        TerminalDialogWidget
                                .showTextFieldDialog(getOs(), "terminal.component.page_name", s -> true, s -> {
                                    if (s != null) {
                                        addTab(s);
                                    }
                                }).setClientSide().open();
                        ...
                        TerminalDialogWidget
                                .showInfoDialog(getOs(), "terminal.component.warning", "terminal.recipe_chart.limit")
                                .setClientSide().open();
                    }
                });
        ClickComponent deletePage = new ClickComponent().setIcon(GuiTextures.ICON_REMOVE)
                .setHoverText("terminal.recipe_chart.delete").setClickConsumer(cd -> {
                    if (tabGroup == null) return;
                    if (tabGroup.getAllTag().size() > 1) {
                        TerminalDialogWidget.showConfirmDialog(getOs(), "terminal.recipe_chart.delete",
                                "terminal.component.confirm", r -> {
                                    if (r) {
        ...
        ClickComponent addSlot = new ClickComponent().setIcon(GuiTextures.ICON_ADD)
                .setHoverText("terminal.recipe_chart.add_slot").setClickConsumer(cd -> {
                    if (tabGroup == null) return;
        ...
        ClickComponent importPage = new ClickComponent().setIcon(GuiTextures.ICON_LOAD)
                .setHoverText("terminal.component.load_file").setClickConsumer(cd -> {
                    ...
                    if (tabGroup.getAllTag().size() < getMaxPages()) {
                        ...
                        TerminalDialogWidget
                                .showFileDialog(getOs(), "terminal.component.load_file", file, true, result -> {
                                    if (result != null && result.isFile()) {
                                        try {
                                            ...
                                            TerminalDialogWidget
                                                    .showInfoDialog(getOs(), "terminal.component.error",
                                                            "terminal.component.load_file.error")
                                                    .setClientSide().open();
                                        }
                                    }
                                }).setClientSide().open();
        ...
        ClickComponent exportPage = new ClickComponent().setIcon(GuiTextures.ICON_SAVE)
                .setHoverText("terminal.component.save_file").setClickConsumer(cd -> {
                    ...
                    if (tabGroup.getCurrentTag() != null) {
                        File file = new File(TerminalRegistry.TERMINAL_PATH, "recipe_chart");
                        TerminalDialogWidget
                                .showFileDialog(getOs(), "terminal.component.save_file", file, false, result -> {
                                    if (result != null) {
                                        try {
                                            CompressedStreamTools.safeWrite(tabGroup.getCurrentTag().saveAsNBT(),
                                                    result);
                                        } catch (IOException e) {
                                            TerminalDialogWidget
                                                    .showInfoDialog(getOs(), "terminal.component.error",
                                                            "terminal.component.save_file.error")
                                                    .setClientSide().open();
                                        }
                                    }
                                }).setClientSide().open();
    ...
    public String transferRecipe(ModularUIContainer container, IRecipeLayout recipeLayout, EntityPlayer player,
        ...
        for (Widget widget : getContainedWidgets(false)) {
            if (widget instanceof RGNode &&
                    ((RGNode) widget).transferRecipe(container, recipeLayout, player, maxTransfer, doTransfer)) {
                return null;
            }
        }

...
public class LaserRenderer implements ICCBlockRenderer, IItemRenderer {
    public static ModelResourceLocation MODEL_LOCATION = new ModelResourceLocation(new ResourceLocation(GTValues.MODID, "laser"), "normal");
    ...
    public static void preInit() {
        BLOCK_RENDER_TYPE = BlockRenderingRegistry.createRenderType("gt_laser");
    ...
    public void onModelsBake(ModelBakeEvent event) {
        GTLog.logger.info("Injected laser render model");
    ...
    private static void renderCableCube(int connections, CCRenderState renderState, IVertexOperation[] pipeline, IVertexOperation[] wire, IVertexOperation[] overlays, EnumFacing side, float thickness) {
        if ((connections & 1 << side.getIndex()) > 0) {
            ...
            Cuboid6 cuboid6 = BlockLaser.getSideBox(side, thickness);
            for (EnumFacing renderedSide : EnumFacing.VALUES) {
    ...
    public void handleRenderBlockDamage(IBlockAccess world, BlockPos pos, IBlockState state, TextureAtlasSprite sprite, BufferBuilder buffer) {
        ...
        BlockLaser blockCable = (BlockLaser) state.getBlock();
        IPipeTile<LaserSize, LaserProperties> tileEntityCable = blockCable.getPipeTileEntity(world, pos);
        ...
        for (EnumFacing renderSide : EnumFacing.VALUES) {
            if ((connectedSidesMask & (1 << renderSide.getIndex())) > 0) {
                Cuboid6 sideBox = BlockLaser.getSideBox(renderSide, thickness);
    ...
    public Pair<TextureAtlasSprite, Integer> getParticleTexture(IPipeTile<LaserSize, LaserProperties> tileEntity) {
        if (tileEntity == null) {

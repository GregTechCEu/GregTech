...
public final class ColorQuadCache {
    ...
    private final RegeneratingSoftReference<Map<ColorData, List<BakedQuad>>> cache;

    public ColorQuadCache(List<RecolorableBakedQuad> prototypes) {
        this.prototypes = prototypes;
        this.cache = new RegeneratingSoftReference<>(Object2ObjectOpenHashMap::new);
    }

    public List<BakedQuad> getQuads(ColorData data) {
        List<BakedQuad> existing = cache.get().get(data);
        if (existing == null) {
            existing = new ObjectArrayList<>();
            for (RecolorableBakedQuad quad : prototypes) {
                existing.add(quad.withColor(data));
            }
            cache.get().put(data, existing);
        }
        return existing;
    }
}

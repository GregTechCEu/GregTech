...
public class ClientProxy extends CommonProxy {
    ...
    private static final Set<UUID> capeHoldersUUIDs = new ObjectOpenHashSet<>();
    ...
    @SubscribeEvent
    public void onRenderPlayerTick(RenderPlayerEvent.Pre event) {
        AbstractClientPlayer player = (AbstractClientPlayer) event.getEntityPlayer();
        if (capeHoldersUUIDs.contains(player.getUniqueID()) && player.hasPlayerInfo() && !Objects.equals(player.getLocationCape(), GREGTECH_CAPE_TEXTURE)) {
            NetworkPlayerInfo networkPlayerInfo = Minecraft.getMinecraft().getConnection().getPlayerInfo(player.getUniqueID());
            Map<Type, ResourceLocation> playerTextures = ObfuscationReflectionHelper.getPrivateValue(NetworkPlayerInfo.class, networkPlayerInfo, 1);
            playerTextures.put(Type.CAPE, GREGTECH_CAPE_TEXTURE);
        }
    }

    @SubscribeEvent
    public void addMaterialFormulaHandler(ItemTooltipEvent event) {
    ...
    @SubscribeEvent
    public void addNBTClearingTooltip(ItemTooltipEvent event) {
    ...
    private static void loadCapesList() {
        ...
        } catch (UnknownHostException | SocketTimeoutException | MalformedURLException ignored) {

...
public class BlockVisitor implements Opcodes {
    ...
    public static final ObfMapping TARGET_METHOD = new ObfMapping(TARGET_CLASS_NAME, "canRenderInLayer",
    ...
    public static ClassNode handleClassNode(ClassNode classNode) {
        for (MethodNode m : classNode.methods) {
            if (m.name.equals(TARGET_METHOD.s_name) && m.desc.equals(TARGET_METHOD.s_desc)) {
                ...
                // Invoke hook
                toAdd.add(new MethodInsnNode(INVOKESTATIC, BLOCK_HOOKS_OWNER, BLOCK_HOOKS_METHOD_NAME,
                        BLOCK_HOOKS_SIGNATURE, false));
                toAdd.add(new InsnNode(DUP)); // Copy value on stack, avoids need for local var
                toAdd.add(new JumpInsnNode(IFNULL, (LabelNode) m.instructions.getFirst())); // Check if return is null,
                                                                                            // if it is, jump to vanilla
                                                                                            // code
                toAdd.add(new MethodInsnNode(INVOKEVIRTUAL, "java/lang/Boolean", "booleanValue", "()Z", false)); // Otherwise
                                                                                                                 // evaluate
                                                                                                                 // the
                                                                                                                 // bool

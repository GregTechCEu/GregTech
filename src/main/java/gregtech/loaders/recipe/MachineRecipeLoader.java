...
//            .duration(72000).EUt(VA[HV]).buildAndRegister();
    ...
    private static void registerAssemblerRecipes() {
        ...
        ASSEMBLER_RECIPES.recipeBuilder().EUt(16).input(plate, HSSG, 6).input(frameGt, HSSG).circuitMeta(6).outputs(MetaBlocks.METAL_CASING.getItemVariant(MetalCasingType.HSSG_STURDY, 2)).duration(50).buildAndRegister();
        ASSEMBLER_RECIPES.recipeBuilder().EUt(16).input(plate, HSSS, 6).input(frameGt, HSSS).circuitMeta(6).outputs(MetaBlocks.METAL_CASING.getItemVariant(MetalCasingType.HSSS_RIGID, 2)).duration(50).buildAndRegister();

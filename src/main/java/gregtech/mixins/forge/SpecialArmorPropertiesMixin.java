...
public class SpecialArmorPropertiesMixin {
    ...
    @ModifyExpressionValue(method = "applyArmor", at = @At(value = "INVOKE_ASSIGN", target = "Lnet/minecraft/util/CombatRules;getDamageAfterAbsorb(FFF)F"), remap = false)
    private static double adjustArmorAbsorption(double originalDamage, float damage, float totalArmor, float totalToughness, @Local EntityLivingBase entity,
                                                @Local NonNullList<ItemStack> inventory, @Local DamageSource damageSource) {

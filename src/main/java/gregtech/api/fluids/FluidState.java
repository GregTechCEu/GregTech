...
public enum FluidState implements ContainmentFailureHandler {
    ...
    public void handleFailure(World world, BlockPos failingBlock, FluidStack failingStack) {
        ...
        switch (this) {
            default -> {
                ...
                for (EntityLivingBase entity : entities) {
                    EntityDamageUtil.applyTemperatureDamage(entity,
                            failingStack.getFluid().getTemperature(failingStack),
                            scalar, 20);
                }
                world.setBlockToAir(failingBlock);
            }
            case GAS -> {
                GTUtility.spawnParticles(world, EnumFacing.UP, EnumParticleTypes.SMOKE_LARGE, failingBlock,
            ...
            case PLASMA -> {
                ...
                world.setBlockToAir(failingBlock);
                world.createExplosion(null, failingBlock.getX() + 0.5, failingBlock.getY() + 0.5,
    ...
    public void handleFailure(EntityPlayer failingPlayer, FluidStack failingStack) {
        ...
        world.playSound(null, failingPlayer.getPosition(), SoundEvents.BLOCK_LAVA_EXTINGUISH, SoundCategory.BLOCKS,
                1.0F, 1.0F);
        switch (this) {
            default -> {
                for (EnumFacing facing : EnumFacing.HORIZONTALS) {
            ...
            case GAS -> {
                GTUtility.spawnParticles(world, EnumFacing.UP, EnumParticleTypes.SMOKE_LARGE, failingPlayer,
                        9 + GTValues.RNG.nextInt(3));
                float scalar = (float) Math.log(failingStack.amount);
                List<EntityLivingBase> entities = world.getEntitiesWithinAABB(EntityLivingBase.class,

...
public class MetaFluids {
    ...
    public static Fluid registerFluid(@Nonnull Material material, @Nonnull FluidType fluidType, int temperature, boolean generateBlock) {
        ...
        if (fluid == null) {
            ...
            if (fluidType.equals(FluidTypes.PLASMA)) {
                // use a custom texture if it exists, else use the universal plasma texture for plasmas
                textureLocation = fluidTextureMap.getOrDefault(Pair.of(material, fluidType), AUTO_GENERATED_PLASMA_TEXTURE);
            } else {
                // use a custom texture if it exists, else use the material's iconset fluid texture

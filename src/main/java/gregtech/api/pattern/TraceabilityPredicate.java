...
public class TraceabilityPredicate {
    ...
    public static Supplier<TraceabilityPredicate> REFRACTORY_BRICKS = () -> new TraceabilityPredicate(blockWorldState -> {
        IBlockState blockState = blockWorldState.getBlockState();
        if (GregTechAPI.REFRACTORY_BRICKS.containsKey(blockState)) {
            IRefractoryBrickBlockStats stats = GregTechAPI.REFRACTORY_BRICKS.get(blockState);
            Object currentRefractoryBrick = blockWorldState.getMatchContext().getOrPut("RefractoryBrickType", stats);
            if (!currentRefractoryBrick.equals(stats)) {
                blockWorldState.setError(new PatternStringError("gregtech.multiblock.pattern.error.refractory_bricks"));
                return false;
            }
            blockWorldState.getMatchContext().getOrPut("VABlock", new LinkedList<>()).add(blockWorldState.getPos());
            return true;
        ...
        return false;
            ...
            .map(entry -> new BlockInfo(entry.getKey(), null))
            .toArray(BlockInfo[]::new))
            .addTooltips("gregtech.multiblock.pattern.error.refractory_bricks");

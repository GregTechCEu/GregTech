package gregtech.api.graphnet.pipenetold.tile;
...
public abstract class TileEntityPipeBase<PipeType extends Enum<PipeType> & IPipeType<NodeDataType>,
        NodeDataType extends IPipeNetData<NodeDataType>, Edge extends NetEdge> extends NeighborCacheTileEntityBase
     ...
     */
    protected @Nullable PipeNetNode<PipeType, NodeDataType, Edge> pipeNetNode;
    ...
    public void transferDataFrom(IPipeTile<PipeType, NodeDataType, Edge> tileEntity) {
        ...
        this.pipeNetNode = tileEntity.getNode();
        this.pipeNetNode.setData(getPipeBlock().createProperties(this));
    ...
    @Override
    public PipeNetNode<PipeType, NodeDataType, Edge> getNode() {
        if (pipeNetNode == null) {
            if (this.getPipeWorld().isRemote)
                pipeNetNode = new PipeNetNode<>(this);
            else pipeNetNode = this.getPipeBlock().getWorldPipeNet(this.getPipeWorld()).getOrCreateNode(this);
        }
        return pipeNetNode;
    }
    ...
    public NodeDataType getNodeData() {
        ...
        if (this.pipeNetNode == null) return getPipeBlock().createProperties(this);
    ...
    protected void doOldNetSetup() {
        ...
        for (EnumFacing facing : EnumFacing.VALUES) {
            PipeNetNode<PipeType, NodeDataType, Edge> nodeOffset = net.getNode(this.getPipePos().offset(facing));

...
public class ParticleHandlerUtil {
    ...
    public static void addBlockRunningEffects(World worldObj, Entity entity, TextureAtlasSprite atlasSprite,
        ...
        DigIconParticle digIconParticle = new DigIconParticle(worldObj, posX, posY, posZ, -entity.motionX * 4.0, 1.5,
                -entity.motionZ * 4.0, atlasSprite);
        digIconParticle.setRBGColorF(red, green, blue);
        manager.addEffect(digIconParticle);
    }

    public static void addBlockLandingEffects(World worldObj, Vector3 entityPos, TextureAtlasSprite atlasSprite,
                                              int spriteColor, ParticleManager manager, int numParticles) {
        if (atlasSprite == null) return;
        ...
        if (traceResult != null && traceResult.typeOfHit == Type.BLOCK && numParticles != 0) {
            for (int i = 0; i < numParticles; i++) {
                ...
                DigIconParticle digIconParticle = DigIconParticle.newLandingParticle(worldObj, entityPos.x, entityPos.y,
                        entityPos.z, mX, mY, mZ, atlasSprite);
                digIconParticle.setRBGColorF(red, green, blue);
                manager.addEffect(digIconParticle);
            }
        }
    ...
    public static void addBlockDestroyEffects(World worldObj, CuboidRayTraceResult result,
                                              TextureAtlasSprite atlasSprite, int spriteColor,
                                              ParticleManager manager) {
        addBlockDestroyEffects(worldObj, result.cuboid6, atlasSprite, spriteColor, manager);
    }

    public static void addBlockDestroyEffects(IBlockState blockState, World worldObj, BlockPos blockPos,
                                              TextureAtlasSprite atlasSprite, int spriteColor,
                                              ParticleManager manager) {
    ...
    public static void addHitEffects(World worldObj, CuboidRayTraceResult result, TextureAtlasSprite atlasSprite,
                                     int spriteColor, ParticleManager manager) {
        addBlockHitEffects(worldObj, result.cuboid6, result.sideHit, atlasSprite, spriteColor, manager);
    }

    public static void addHitEffects(IBlockState blockState, World worldObj, RayTraceResult target,
                                     TextureAtlasSprite atlasSprite, int spriteColor, ParticleManager manager) {
    ...
    public static void addBlockHitEffects(World world, Cuboid6 bounds, EnumFacing side, TextureAtlasSprite icon,
                                          int spriteColor, ParticleManager particleManager) {
        if (icon == null) return;
        float border = 0.1F;
    ...
    public static void addBlockDestroyEffects(World world, Cuboid6 bounds, TextureAtlasSprite icon, int spriteColor,
        ...
        for (int i = 0; i < density.x; ++i) {
            for (int j = 0; j < density.y; ++j) {
                for (int k = 0; k < density.z; ++k) {
                    ...
                    DigIconParticle digIconParticle = new DigIconParticle(world, x, y, z, x - center.x, y - center.y,
                            z - center.z, icon);
                    digIconParticle.setRBGColorF(red, green, blue);
                    particleManager.addEffect(digIconParticle);
                }

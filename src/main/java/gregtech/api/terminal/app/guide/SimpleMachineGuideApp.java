...
public class SimpleMachineGuideApp extends GuideApp<MetaTileEntity> {
    ...
    public static void registerSimpleMachine(MetaTileEntity mte) {
        if (FMLCommonHandler.instance().getSide().isClient()) {
            JsonObject config = getConfig("terminal/guide/simplemachines/" + mte.metaTileEntityId.getPath() + ".json");
            if (config == null) {

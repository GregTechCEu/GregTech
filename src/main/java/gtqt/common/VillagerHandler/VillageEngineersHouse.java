...
import static gregtech.common.items.MetaItems.CREDIT_COPPER;
...
public class VillageEngineersHouse extends StructureVillagePieces.Village {
    ...
    public boolean addComponentParts(World world, Random rand, StructureBoundingBox box)
    {
        ...
        try
        {
            ...
            this.placeItemframe(rand, world, 4, 3, 2, getCoordBaseMode().getOpposite(), CREDIT_COPPER.getStackForm());
        } catch(Exception e)
        {
            e.printStackTrace();
        }
    ...
    protected boolean placeCrate(World world, StructureBoundingBox box, Random rand, int x, int y, int z)
    {
        ...
        if(box.isVecInside(pos)&&(world.getBlockState(pos)!=Blocks.CHEST.getStateFromMeta(0)))
        {
            world.setBlockState(pos, Blocks.CHEST.getDefaultState(), 2);
            TileEntity tile = world.getTileEntity(pos);
            if(tile instanceof TileEntityChest)
                ((TileEntityChest)tile).setLootTable(LootTableList.CHESTS_VILLAGE_BLACKSMITH, rand.nextLong());
            return true;
        }
        else
            return false;
    ...
    public void placeItemframe(Random random, World world, int x, int y, int z, EnumFacing side, ItemStack stack)
    {
        ...
        if(e.onValidSurface()&&world.getEntitiesWithinAABB(
                EntityHanging.class, new AxisAlignedBB(i1-.125, j1, k1-.125, i1+1.125, j1+1, k1+1.125)).isEmpty())
            if(!world.isRemote)
                world.spawnEntity(e);
    }
    ...
    {
        return VillagerHandler.FOLK_SCIENTIST_PROFESSION;
    }
    ...
    public static class VillageManager implements VillagerRegistry.IVillageCreationHandler
    {
        ...
        {
            return new StructureVillagePieces.PieceWeight(VillageEngineersHouse.class, 15, MathHelper.getInt(random, 0+i, 1+i));
        }

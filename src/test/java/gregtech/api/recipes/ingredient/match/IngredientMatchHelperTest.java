...
public class IngredientMatchHelperTest {
    ...
    public void test() {
        ...
        assert IngredientMatchHelper.matchItems(matchers, stacks) == null;

        wood.setCount(15);
        // should succeed with these exact consumptions
        long[] match = IngredientMatchHelper.matchItems(matchers, stacks);
        assert match != null;
        assert match[0] == 10;
        assert match[1] == 5;
        assert match[2] == 2;
        ...
        stacks.add(wool2);
        // should succeed with these exact consumptions
        match = IngredientMatchHelper.matchItems(matchers, stacks);
        assert match != null;
        assert match[0] == 10;
